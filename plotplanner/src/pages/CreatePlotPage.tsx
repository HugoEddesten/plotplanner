...
export default function CreatePlotPage() {
  ...
  return (
    <div className="min-h-screen bg-[--color-bg]">
      ...
      <Container size={step === "shape" ? "lg" : "xs"} py={56}>
            ...
            <div key={s} className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{
                    background: step === s || (s === "name" && step === "shape")
                      ? "var(--color-primary)"
                      : "var(--earth-300)",
                    color: step === s || (s === "name" && step === "shape") ? "white" : "var(--sage-500)",
                  }}
                >
                  {s === "name" && step === "shape" ? "✓" : i + 1}
                </div>
                <Text size="sm" fw={step === s ? 600 : 400} className={step === s ? "text-primary-dark" : "text-muted"}>
        ...
        {step === "name" && (
          <Stack gap="xl">
            ...
            <Group>
              <Button variant="subtle" radius="xl" className="text-muted!" onClick={() => navigate("/dashboard")}>
                Cancel
              </Button>
        ...
        {step === "shape" && (
          <Stack gap="xl">
            <div>
              ...
              <Text className="text-muted mt-1">
                Sketch the boundary of <strong>{name}</strong> by placing corner points on the canvas.
                You can skip this and draw it later.
              </Text>
            </div>

            <PolygonCanvas points={points} onChange={setPoints} />
            ...
            <Group justify="space-between">
              <Group gap="sm">
                <Button variant="subtle" radius="xl" className="text-muted!" onClick={() => setStep("name")}>
                  ← Back
                </Button>
                {points.length > 0 && (
                  <Button variant="subtle" radius="xl" className="text-muted!" onClick={() => setPoints([])}>
                    Clear
                  </Button>
                )}
              </Group>
              <Button radius="xl" className="bg-primary!" loading={isPending} onClick={handleCreate}>
                Create plot
              </Button>
            </Group>
          </Stack>
