...
func (r *Repository) PlantOnCell(plotId int, plantId *int, userPlantId *int, col, row int) (*PlotPlant, error) {
	...
	err := r.DB.QueryRow(context.Background(),
		`INSERT INTO plot_plants (plot_id, plant_id, user_plant_id, col, row)
		 VALUES ($1, $2, $3, $4, $5)
		 ON CONFLICT (plot_id, col, row) DO UPDATE
		   SET plant_id = EXCLUDED.plant_id,
		       user_plant_id = EXCLUDED.user_plant_id,
		       updated_at = NOW()
		 RETURNING id, plot_id, plant_id, user_plant_id, col, row, created_at, updated_at`,
		plotId, plantId, userPlantId, col, row,
	).Scan(&pp.Id, &pp.PlotId, &pp.PlantId, &pp.UserPlantId, &pp.Col, &pp.Row, &pp.CreatedAt, &pp.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) ListForPlot(plotId int) ([]PlotPlant, error) {
	rows, err := r.DB.Query(context.Background(),
		`SELECT pp.id, pp.plot_id, pp.plant_id, pp.user_plant_id, pp.col, pp.row, pp.created_at, pp.updated_at,
	...
	for rows.Next() {
		var pp PlotPlant
		if err := rows.Scan(&pp.Id, &pp.PlotId, &pp.PlantId, &pp.UserPlantId, &pp.Col, &pp.Row, &pp.CreatedAt, &pp.UpdatedAt, &pp.PlantName); err != nil {
			return nil, err
		}
...
func (r *Repository) FindByIdForUser(plotPlantId, userId int) (*PlotPlant, error) {
	...
	err := r.DB.QueryRow(context.Background(),
		...
		plotPlantId, userId,
	).Scan(&pp.Id, &pp.PlotId, &pp.PlantId, &pp.UserPlantId, &pp.Col, &pp.Row, &pp.CreatedAt, &pp.UpdatedAt, &pp.PlantName)
	if err != nil {
		return nil, err
	}
	return &pp, nil
...
func (r *Repository) RemoveFromCell(plotId, col, row int) error {
	_, err := r.DB.Exec(context.Background(),
		`DELETE FROM plot_plants WHERE plot_id = $1 AND col = $2 AND row = $3`,
		plotId, col, row,
	)
	return err
}
